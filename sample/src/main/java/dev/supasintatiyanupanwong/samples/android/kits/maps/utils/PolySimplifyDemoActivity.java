...
public class PolySimplifyDemoActivity extends BaseDemoActivity {
    ...
    protected void startDemo(boolean isRestore) {
        MapClient map = getMap();

        // Original line
        List<LatLng> line = PolyUtils.decode(LINE);
        ...
        if (!isRestore) {
            map.moveCamera(
                    MapKit.getCameraUpdateFactory()
                            .newLatLngZoom(MapKit.newLatLng(28.05870, -82.4090), 15)
            );
        }
        ...
        simplifiedLine = PolyUtils.simplify(line, tolerance);
        map.addPolyline(MapKit.newPolylineOptions()
        ...
        triangle.add(MapKit.newLatLng(28.06025,-82.41030));  // Should match last point
        triangle.add(MapKit.newLatLng(28.06129,-82.40945));
        triangle.add(MapKit.newLatLng(28.06206,-82.40917));
        triangle.add(MapKit.newLatLng(28.06125,-82.40850));
        triangle.add(MapKit.newLatLng(28.06035,-82.40834));
        triangle.add(MapKit.newLatLng(28.06038, -82.40924));
        triangle.add(MapKit.newLatLng(28.06025,-82.41030));  // Should match first point

        map.addPolygon(MapKit.newPolygonOptions()
                .addAll(triangle)
        ...
        tolerance = 88; // meters
        List<LatLng> simplifiedTriangle = PolyUtils.simplify(triangle, tolerance);
        map.addPolygon(MapKit.newPolygonOptions()
        ...
        List<LatLng> oval = PolyUtils.decode(OVAL_POLYGON);
        map.addPolygon(MapKit.newPolygonOptions()
                .addAll(oval)
                .fillColor(Color.BLUE - ALPHA_ADJUSTMENT)
                .strokeColor(Color.BLUE)
        ...
        tolerance = 10; // meters
        List<LatLng> simplifiedOval= PolyUtils.simplify(oval, tolerance);
        map.addPolygon(MapKit.newPolygonOptions()
