...
public class GeoJsonDemoActivity extends BaseDemoActivity {
    ...
    private static float magnitudeToColor(double magnitude) {
        if (magnitude < 1.0) {
            return BitmapDescriptor.Factory.HUE_CYAN;
        } else if (magnitude < 2.5) {
            return BitmapDescriptor.Factory.HUE_GREEN;
        } else if (magnitude < 4.5) {
            return BitmapDescriptor.Factory.HUE_YELLOW;
        } else {
        ...
        if (!isRestore) {
            getMap().moveCamera(
                    MapKit.getCameraUpdateFactory()
                            .newLatLng(MapKit.newLatLng(31.4118,-103.5355))
            );
        }
    ...
    private void addColorsToMarkers(GeoJsonLayer layer) {
        ...
        for (GeoJsonFeature feature : layer.getFeatures()) {
            ...
            if (feature.getProperty("mag") != null && feature.hasProperty("place")) {
                ...
                BitmapDescriptor pointIcon = MapKit.getBitmapDescriptorFactory()
                        .defaultMarker(magnitudeToColor(magnitude));

                // Create a new point style
                GeoJsonPoint.Style pointStyle = new GeoJsonPoint.Style();
