...
import me.tatiyanupanwong.supasin.android.samples.kits.maps.utils.model.Person;
...
public class CustomMarkerClusteringDemoActivity extends BaseDemoActivity implements
    ...
    private class PersonRenderer extends DefaultClusterRenderer<Person> {
        ...
        private final IconGenerator mClusterIconGenerator =
                new IconGenerator(getApplicationContext());
        ...
        protected void onBeforeClusterItemRendered(
                @NonNull Person person, @NonNull Marker.Options markerOptions) {
            // Draw a single person - show their profile photo and set the info window to show
            // their name
            markerOptions
        ...
        private BitmapDescriptor getItemIcon(Person person) {
            ...
            return MapKit.getBitmapDescriptorFactory().fromBitmap(icon);
        }

        @Override
        protected void onBeforeClusterRendered(
                @NonNull Cluster<Person> cluster, @NonNull Marker.Options markerOptions) {
    ...
    public boolean onClusterClick(Cluster<Person> cluster) {
        ...
        Toast.makeText(
                this, cluster.getSize() + " (including " + firstName + ")", Toast.LENGTH_SHORT)
                .show();
        ...
        try {
            getMap().animateCamera(MapKit.getCameraUpdateFactory().newLatLngBounds(bounds, 100));
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        if (!isRestore) {
            getMap().moveCamera(
                    MapKit.getCameraUpdateFactory()
                            .newLatLngZoom(MapKit.newLatLng(51.503186, -0.126446), 9.5f)
            );
        }
    ...
    private LatLng position() {
        return MapKit.newLatLng(
                random(51.6723432, 51.38494009999999), random(0.148271, -0.3514683));
    }
